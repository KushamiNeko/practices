...
additional_cols = [
    find_col(df, ["coccoides"]),
    find_col(df, ["leptum"]),
    find_col(df, ["b", "fragilis"]),
    find_col(df, ["bifidobacte"]),
    find_col(df, ["atopobium"]),
    find_col(df, ["prevotella"]),
    find_col(df, ["total", "lacto"]),
    find_col(df, ["enterob"]),
    find_col(df, ["enterococcu"]),
    find_col(df, ["staphylo"]),
    find_col(df, ["c", "difficile"]),
    find_col(df, ["c", "perfri"]),
]
...
for colx in df.columns:
    ...
    for coly in df.columns:
        ...
        tau, p = delta_corr(df, colx, coly, "mb")
        if p <= p_threshold and abs(tau) >= tau_threshold:
            delta_corr_6.append((colx, coly, p, tau))

        tau, p = delta_corr(df, colx, coly, "fb")
        if p <= p_threshold and abs(tau) >= tau_threshold:
            delta_corr_12.append((colx, coly, p, tau))

        tau, p = delta_corr(df, colx, coly, "mbfb")
        if p <= p_threshold and abs(tau) >= tau_threshold:
            delta_corr_6_12.append((colx, coly, p, tau))

        tau, p = delta_corr(df, colx, coly, "bbmbfb")
        if p <= p_threshold and abs(tau) >= tau_threshold:
            delta_corr_0_6_12.append((colx, coly, p, tau))
...
#################################
# using delta correlation instead
#################################
...
for x in df.columns:
    ...
    for y in df.columns:
        ...
        tau, p_value = correlation(df, x, y)
